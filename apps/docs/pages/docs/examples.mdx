...
function Carousel() {
  ...
  const totalPages = Math.ceil(data.length / 3) // Assuming 3 items per page
  ...
  return (
    <div className="carousel-container">
      ...
      <div className="carousel-controls">
        <button 
          onClick={() => onScrollTo(Math.max(0, page - 1))}
          disabled={page === 0}
        >
          Previous
        </button>
  ...
  return (
    <div>
      <div className="navigation-controls">
        ...
        <button onClick={() => onScrollTo(pages - 1)}>Last Page</button>
      </div>
...
function AccessibleGrid() {
  ...
  return (
    <div
      ...
      aria-rowcount={Math.ceil(data.length / columns)}
